...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://halliekaufman@localhost/hkaufHW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
search_gifs = db.Table('Search_Gifs', db.Column('gif_id', db.Integer, db.ForeignKey('Gifs.id')), db.Column('term', db.String(32), db.ForeignKey('SearchTerm.term')))
...
user_collection = db.Table('user_collection', db.Column('gif_id', db.Integer, db.ForeignKey('Gifs.id')), db.Column('collection_id', db.Integer, db.ForeignKey('PersonalGifCollection.id')))
...
class Gif(db.Model):
    ...
    def __repr__(self):
        return 'Gif title: {}, URL: {}'.format(self.title, self.embedURL)
...
def get_or_create_search_term(db_session, term):
    ...
    if searchterm:
        ...
        searching = get_gifs_from_giphy(term)
        for gif in searching:
            g = get_or_create_gif(db_session, title = gif['title'], url = gif['embed_url'])
            searchterm.gifs.append(g)
...
def create_collection():
    ...
    if request.method == 'POST':
        selectedgif = form.gif_picks.data
        gif_obj = [get_gif_by_id(int(id)) for id in selectedgif]
        get_or_create_collection(db.session, name = form.name.data, current_user = current_user, gif_list = gif_obj)
